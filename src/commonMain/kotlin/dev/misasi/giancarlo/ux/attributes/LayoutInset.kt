...
data class LayoutInset (val margin: Inset = Inset(), val padding: Inset = Inset()) {
    val horizontal by lazy { margin.horizontal.plus(padding.horizontal) }
    val vertical by lazy { margin.vertical.plus(padding.vertical) }
    val size by lazy { Vector2i(horizontal, vertical) }
}